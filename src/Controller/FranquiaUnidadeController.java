...
//        return false;
    ...
    public static void alteraUniFranq(FranquiaUnidade edit, int opc){
        if(opc == 1){
            try(Connection con = new DBConnect().getConnection(); 
                PreparedStatement stmt = con.prepareStatement("update unidade_franquia set id_franquia = ?, data_modificacao = current_timestamp() where id = ?;")){
                
                stmt.setInt(1, (int) edit.getFranquia());
                stmt.setInt(2, (int) edit.getId());
                
                stmt.execute();
                
                System.out.println("Unidade de franquia alterada com sucesso");
        ...
        if(opc == 2){
            try(Connection con = new DBConnect().getConnection(); 
                PreparedStatement stmt = con.prepareStatement("update unidade_franquia set cidade = ?, data_modificacao = current_timestamp() where id = ?;")){
        ...
        if(opc == 3){
            try(Connection con = new DBConnect().getConnection(); 
                PreparedStatement stmt = con.prepareStatement("update unidade_franquia set endereco = ?, data_modificacao = current_timestamp() where id = ?;")){
        ...
        if(opc == 4){
            try(Connection con = new DBConnect().getConnection(); 
                PreparedStatement stmt = con.prepareStatement("update unidade_franquia set id_responsavel = ?, data_modificacao = current_timestamp() where id = ?;")){
